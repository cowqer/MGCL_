...
class Evaluator:
    ...
    @staticmethod
    def visualize_batch_mask_only(pred_label, batch, save_dir="./vis_results", target_classes=None):
        """
        只保存黑白 mask（前景=255，背景=0）
        pred_label: torch.Tensor (B,H,W)
        batch: dict, 包含 query_label, query_name, class_id
        ...
        query_label = batch.get('query_label', None)     # (B,H,W)
        query_names = batch.get('query_name', None)      # list[str]
        class_ids   = batch.get('class_id', None)        # (B,)
        ...
        for i in range(B):
            # 如果指定了 target_classes，则只对这些类可视化
            if target_classes is not None and class_ids is not None:
                if int(class_ids[i]) not in target_classes:
                    continue

            name = query_names[i] if query_names is not None else f"img_{i}"
            pred_np = pred_label[i].cpu().numpy().astype(np.uint8)
            gt_np   = query_label[i].cpu().numpy().astype(np.uint8)


            pred_bw = to_binary(pred_np)
            gt_bw   = to_binary(gt_np)

            # 保存黑白 mask
            Image.fromarray(pred_bw).save(os.path.join(save_dir, f"{name}_pred.png"))
            Image.fromarray(gt_bw).save(os.path.join(save_dir, f"{name}_gt.png"))

    # @staticmethod
    # def visualize_batch_mask_only(pred_label, batch, save_dir="./vis_results"):
    #     """
    #     只保存黑白 mask（前景=255，背景=0）
    #     pred_label: torch.Tensor (B,H,W)
    #     batch: dict, 包含 query_label 和 query_name
    #     """
    #     os.makedirs(save_dir, exist_ok=True)
...
class DatasetLCMA(Dataset):
    ...
    def sample_episode(self, idx):
        class_sample = self.class_ids[idx % len(self.class_ids)]  # 不管 train/val 都从 class_ids 取
        class_list = self.img_metadata_classwise[class_sample]
        last_class = self.class_ids[-1]

        # --------- query 采样 ---------
        if class_sample == last_class:
            # query 固定从前 30 张里采
            query_name = np.random.choice(class_list[:50], 1, replace=False)[0]
            # print(f"[DEBUG] query_name: {query_name}, class_sample: {class_sample}")
        else:
            query_name = np.random.choice(class_list, 1, replace=False)[0]

        # --------- support 采样 ---------
        if class_sample == last_class:
            support_pool = class_list[50:]  # support 固定从第 30 之后采
            # print(f"[DEBUG] support_pool: {support_pool}, class_sample: {class_sample}")
        else:
            support_pool = class_list

        support_names = []
        
        while True:
            support_name = np.random.choice(support_pool, 1, replace=False)[0]
            if query_name != support_name:  # 避免重合
    ...
    def build_img_metadata(self):
        ...
        def read_metadata(split, fold_id):
            split = "trn" if split == "train" else split
            fold_n_metadata = os.path.join('data/splits/LCMA/%s/fold%d.txt' % (split, fold_id))
            with open(fold_n_metadata, 'r') as f:
                fold_n_metadata = f.read().split('\n')[:-1]
            fold_n_metadata = [[data.split('__')[0], int(data.split('__')[1]) - 1] for data in fold_n_metadata]
            return fold_n_metadata
...
class DatasetLCMA_plus(Dataset):
    ...
    # def get_data_and_mask_bbox(self, pkl_name):
    #     pkl = os.path.join(self.pkl_path, pkl_name) + '.pkl'
    #     with open(pkl, "rb") as f:
    #         image_label_mask = pickle.load(f)
    ...
    #     query_bboxes = np.asarray([mask["bbox"] for mask in image_label_mask["masks_target"]], dtype=np.float32)
    #     return query_img, query_label, query_mask, query_bboxes
    def get_data_and_mask(self, pkl_name):
        pkl = os.path.join(self.pkl_path, pkl_name) + '.pkl'
        with open(pkl, "rb") as f:
            image_label_mask = pickle.load(f)
    ...
    def load_frame(self, query_name, support_names):
        if self.use_mask:
            ...
            support_labels = [one[1] for one in _support_data]
            support_masks = [one[2] for one in _support_data]
            # support_bboxes = [one[3] for one in _support_data]
        else:
            query_img = self.read_img(query_name)
            query_label = self.read_label(query_name)
            query_mask = self.read_label(query_name)
...
class FSSDataset(object):
    ...
    def initialize(cls, img_size, datapath):
        cls.datasets = {'isaid': DatasetISAID, 'dlrsd': DatasetDLRSD, 'lcma_plus': DatasetLCMA_plus,'lcma': DatasetLCMA}
