...
class Evaluator:
    ignore_index = 255 #像素值=255的地方不计入指

    @classmethod
    def classify_prediction(cls, pred_label, batch):#算一个batch里的 预测和GT的交集/并集
...
class DatasetDLRSD(Dataset):
    ...
    def get_data_and_mask(self, pkl_name):#从 .pkl 文件加载图片，标签，SAM生成的多边形mask（rle编码解码）
    ...
    def __len__(self):#返回所有 episode 的数量
