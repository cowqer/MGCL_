...
from .net_tools import MGFEModule
from .modules import CenterPivotConv4d,GatedFusion
...
class SSCDModule(nn.Module):
    ...
    def __init__(self, inch):
        ...
        self.SSblock = SSblock(dim=outch4)
...
class SSCDModule_v1(nn.Module):
    ...
    def __init__(self, inch):
        ...
        # Decoder layers
        self.decoder1 = nn.Sequential(
            nn.Conv2d(192, outch3, (3, 3), padding=(1, 1), bias=True), nn.ReLU(),
            nn.Conv2d(outch3, outch2, (3, 3), padding=(1, 1), bias=True), nn.ReLU())
    ...
    def forward(self, hypercorr_pyramid, query_mask=None):
        ...
        if query_mask is not None:
            _hypercorr_encoded = MGFEModule.update_feature_one(hypercorr_encoded, query_mask)
            _hypercorr_encoded_ = self.SSblock(hypercorr_encoded)         #! SSblock
            hypercorr_encoded = torch.concat([hypercorr_encoded, _hypercorr_encoded, _hypercorr_encoded_], dim=1)
...
class SSCDModule_v2(SSCDModule):
    def __init__(self, inch):
        super().__init__(inch)   # 调用父类的初始化，保持兼容
        outch1, outch2, outch3, outch4 = 16, 32, 64, 128
        self.decoder1 = nn.Sequential(
            nn.Conv2d(outch4 + outch3, outch3, (3, 3), padding=(1, 1), bias=True), nn.ReLU(),
            nn.Conv2d(outch3, outch2, (3, 3), padding=(1, 1), bias=True), nn.ReLU())
        
        self.gated_fusion = GatedFusion(in_channels=outch4)
    ...
    def forward(self, hypercorr_pyramid, query_mask=None):
        ...
        if query_mask is not None:
            _hypercorr_encoded = MGFEModule.update_feature_one(hypercorr_encoded, query_mask)
            _hypercorr_encoded_ = self.SSblock(hypercorr_encoded)         #! SSblock
            fused_feat = self.gated_fusion(_hypercorr_encoded, _hypercorr_encoded_)
            hypercorr_encoded = torch.cat([hypercorr_encoded, fused_feat], dim=1)  # 2 倍通道
        else:
            hypercorr_encoded = torch.concat([hypercorr_encoded, hypercorr_encoded], dim=1)
            pass
...
class SSCDModule_v3(SSCDModule):
    def __init__(self, inch):
        super().__init__(inch)   # 调用父类的初始化，保持兼容
        outch1, outch2, outch3, outch4 = 16, 32, 64, 128
        self.decoder1 = nn.Sequential(
        ...
        self.conv1_1x1 = nn.Conv2d(outch4//2, outch4//4, (1, 1), groups=outch4//2, bias=False)
        self.conv2_1x1 = nn.Conv2d(outch4//2, outch4//4, (1, 1), groups=outch4//2, bias=False)

        
        self.conv_7x7 = nn.Conv2d(2, 2, kernel_size=7, padding=3, bias=False)
    ...
    def forward(self, hypercorr_pyramid, query_mask=None):
        ...
        if query_mask is not None:
            ...
            _hypercorr_encoded = MGFEModule.update_feature_one(hypercorr_encoded, query_mask) #64
            _hypercorr_encoded_2 = self.conv1_1x1(_hypercorr_encoded)#32 
            
            _hypercorr_encoded_ = self.SSblock(hypercorr_encoded)  #64       #! SSblock
            _hypercorr_encoded__2 = self.conv2_1x1(_hypercorr_encoded_)#32
            
            mixed_hypercorr_encoded = torch.cat([_hypercorr_encoded_2,_hypercorr_encoded__2], dim=1)#64
            
            gap = torch.mean(mixed_hypercorr_encoded, dim=1, keepdim=True)       # [B, 1, H, W]
            gmp, _ = torch.max(mixed_hypercorr_encoded, dim=1, keepdim=True)     # [B, 1, H, W]
            
            agg = torch.cat([gap, gmp], dim=1)
            sig = self.conv_squeeze(agg).sigmoid()
            attn = _hypercorr_encoded * sig[:,0,:,:].unsqueeze(1) + _hypercorr_encoded_ * sig[:,1,:,:].unsqueeze(1)

            hypercorr_encoded = torch.cat([hypercorr_encoded, attn], dim=1)  #128
        else:
            hypercorr_encoded = torch.concat([hypercorr_encoded, hypercorr_encoded], dim=1)
            pass
