...
class Config(object):
    ...
    data_root = "/data/seekyou/Data/DLRSD_split"
...
class DLRSDAll(Dataset):
    ...
    def __init__(self, split=None):
        ...
        image_dirs = sorted(glob(os.path.join(Config.data_root, split, "Images")))
        label_dirs = sorted(glob(os.path.join(Config.data_root, split, "Labels")))
    ...
    def runner_1_extracted_mask(self, result_mask_name, mask_num=128, target_size=64):
        for i in tqdm(range(len(self.dataset))):##tqdm进度条显示
            ...
            result_mask_file = Tools.new_dir(os.path.join(
                Config.data_root, self.split, result_mask_name, "{}.pkl".format(now_name))) # 生成保存路径
